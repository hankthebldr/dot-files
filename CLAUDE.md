...
Everything hangs off five contracts. When adding a capability, wire it into
the spine — do not add a parallel dispatcher, palette, dashboard, login
renderer, or destination list.

1. **One dispatcher.** `shell/claw-fn.zsh` defines the ONLY `claw()` shell
   function (in-shell work: bare `claw`/`claw menu` → `claw_palette`,
   `load`/`off`, `dash`, `theme set`, bare-profile shorthand `claw security` ≡
   `claw load security`); everything else passes through to `bin/claw` (bash),
   which routes to `scripts/`. `_claw_load_profile` is the ONE load path and
   `_claw_apply_outcome` the ONE applier — `claw load`, the shorthand and the
   palette all land there. Never define a second `claw()` — a previous one in
   aliases.zsh was dead-shadowed for weeks.
2. **One theme engine.** `scripts/utils/theme.sh` is the single source of
   ...
   refined-dark. `.zshrc` step 2c sources it. It is loaded **at most once per
   process tree** and is **fork-free**: `claw_theme_load` returns immediately
   when `CLAW_THEME_SLUG` is already exported with a palette, so children that
   re-source it inherit the env instead of re-parsing (`CLAW_THEME_FORCE=1`
   is the only way to re-read). It is also the GENERATOR of every derived
   colour artifact — `claw_theme_emit p10k|tui|fzf|osc` (p10k typesets the
   POWERLEVEL9K_* colours, tui the `c_*` escapes, fzf `CLAW_FZF_COLOR`, osc
   the terminal fg/bg/cursor sequence). ALL surfaces consume `CLAW_RGB_*` /
   `CLAW_C_*` / `claw_theme_fzf` / an emitter with refined-dark fallbacks —
   never hardcode hex/ANSI colors and never hand-roll a second emitter (the
   rust TUI reads the same state in `tui/claw-tui/src/theme.rs`).
3. **One render path.** `scripts/utils/claw-dashboard.py` is the ONE renderer,
   with two modes on the same frame/width engine: `--login` (the daily card)
   and `--profile <key>` (the profile card). EVERY profile renders through
   `--profile` — `_claw_load_profile` prefix-assigns the six `PROFILE_*`
   variables and calls it; `CLAW_PROFILE_ART=fastfetch` is the opt-out to the
   per-profile `config-*.jsonc` art, and a plain `✓ loaded profile` line is
   the last resort. The LOGIN prints the attention strip (`_claw_attention_strip`,
   at most 3 item lines plus a `+N more` overflow line, silent when clear) and **at most one card per local day**
   (`CLAW_LOGIN_CARD=always|daily|never`, default daily, day claimed via
   `~/.cache/claw/card.stamp`). Data is ONE `fastfetch --format json` call plus
   pure cache reads (`attention.json`, `situation.json`, `local.json`,
   `homelab.json`) — never a live docker/tailscale/kubectl probe, and no longer
   `ff-readout.sh` (whose render-path consumer is now `claw specs`).
4. **Decide-then-render login.** `.zshrc` may only DECIDE: step 2b sources
   `shell/claw-login.zsh` and calls `claw_login`, which resolves actor +
   profile + theme + group, exports them, and registers a ONE-SHOT `precmd`
   hook. Pure zsh, zero forks, zero output. Anything that RENDERS, PROBES or
   READS THE TTY runs from that hook — after the shell is complete — and is
   gated on the actor: `human`/`ssh`/`unknown` render, `agent`/`ide`/`nested`
   do not (card and OSC are `human` only). Nothing on the login path reads
   stdin, and **fzf exists only in `claw_palette`, never on the login path**.
   Do not move work back into the rc, and do not add a second login renderer.
5. **One registry.** Every destination is declared ONCE — a profile in
   `shell/profiles/<p>/meta.zsh` (`PROFILE_GLYPH`/`PROFILE_DESC`/`PROFILE_TIER`
   + the rest), a verb in `config/claw/actions.tsv` (8 tab-separated columns).
   `scripts/utils/registry.sh` is the ONE emitter over both — `rows`,
   `palette`, `completion`, `ids`, `run`, `show`, `help`, `check` — and every
   menu, help text, completion and stats surface is GENERATED from it
   (`claw help`, `shell/claw-completion.zsh`, `claw_palette`, `claw tui-stats`).
   Never copy the destination list into a new surface. `registry.sh check`
   runs inside `claw profiles lint`, so a duplicate id/alias/glyph, a bad
   flag or a `run` naming a non-existent `bin/claw` arm fails the lint.
...
field), `claw doctor`, and refreshed in the background at login by
`_claw_login_render`'s kick (`shell/claw-login.zsh`). Superseded/uncalled scripts live in `legacy/` (see
...
1. **PATH + `DOTFILES_DIR`** — set **inline** (homebrew, `$HOME/.local/bin`, cargo, go, `scripts/utils`, `bin/claw`). Deliberately inline and FIRST — **there is no `shell/path.zsh`** — so every tool resolves before `claw_login` decides at step 2b. Also arms `trap ':' INT` for the length of the rc (released by the file's last line) so a Ctrl-C during login can never abort it mid-file.
2. `shell/platform.zsh` (cross-platform shims) → `ghostty-terminfo.zsh`
2b. **`claw_login` (decide only; render from the first precmd)** — sources `shell/claw-login.zsh`, resolves actor/profile/theme/group, registers the one-shot `precmd` render hook. Pure zsh, no forks, no output
2c. `scripts/utils/theme.sh` (theme engine, single color source) → 2d. `shell/fastfetch.zsh`
3. **(removed)** — the fzf welcome menu used to run here. Choice now lives in `claw_palette` (bare `claw` / `claw menu` / `^G`), reachable once a prompt exists
4. P10k instant prompt — **disabled on purpose** (the login render provides the immediate visual feedback instant prompt was designed for)
...
8. Profile source + `_claw_profile_cd` (fresh logins only, gated on `_CLAW_FRESH_LOGIN`) → P10k theme config — sources the tuned prompt straight from `shell/.p10k.zsh` in the repo (not `~/.p10k.zsh`), profile-aware; then symlink-drift guard, terraform completion, FZF keybindings, `~/.zshrc.local` overrides, appended external-tool PATH
...
`shell/profiles/` contains context-specific environments loaded via `claw <profile>` / `claw load <profile>`, the palette, or the login pin (`claw pin <profile>` → `~/.config/claw/login-profile`, read by `claw_login`). Each profile is a **directory**, not a single file: a thin 5-line dispatcher `shell/profiles/<name>.zsh` sources `<name>/{meta,common,mac|linux}.zsh` (platform-split so macOS/Linux-only aliases don't leak cross-platform):
...
- `meta.zsh` — the profile's half of the ONE registry: `PROFILE_THEME_DEFAULT`, `PROFILE_START_DIR`, `PROFILE_GLYPH` (unique across the whole registry), `PROFILE_DESC` (≤40 chars), `PROFILE_TIER` (1 core · 2 domain · 3 agent · 4 knowledge · 5 customer · 6 hardware), `PROFILE_CLASS`, `PROFILE_TAG`, `PROFILE_KEY_TOOLS`, `PROFILE_TOOLCHAIN`. One `PROFILE_X="…"` per line — `registry.sh` parses this file with a single awk pass and `claw profiles lint` enforces the grammar
...
ONE applier every load path calls (`claw load`, the bare-profile shorthand,
the palette via `_claw_apply_outcome`, and `.zshrc` step 8 on a fresh login). Never hand-roll a top-level `cd` in a profile file.
...
`claw profiles lint` (`scripts/utils/profiles-lint.sh`) validates the directory-per-profile contract — every profile has the expected files, declares `PROFILE_START_DIR` with a known `@token`, declares a unique `PROFILE_GLYPH` / a `PROFILE_DESC` / an in-range `PROFILE_TIER`, exports the required symbols, and hand-rolls no top-level `cd`. Its last step is `registry.sh check`, so registry violations fail the lint too.

### Fastfetch Profile Configs

`config/.config/fastfetch/` contains:
- `config.jsonc` + `logo.txt` — Generic OPEN CLAW header
...
`scripts/utils/onboarding.sh` — 80s arcade-themed character-creation flow that asks 6 personality questions (work hours, weapon of choice, prod-incident style, dream homelab, etc.), tallies points against the 8 core workflow profiles, announces an RPG-style "class" (e.g. `SKYSURFER` for cloud, `NIGHTHACKER` for security, `NEUROMANCER` for ai), and offers to install the matching toolchain + activate the profile. Surfaced via `claw onboard`, the palette (the `onboard` row in `config/claw/actions.tsv`), and the bootstrap end-of-install banner. Visual palette: hot pink / neon cyan / synthwave purple. Uses gum if available, falls back to plain `read`.
...
- **Surfaced**: `claw clin {sync|install|setup}` dispatch, a "Clin Notes" row in the palette (the `clin` action in `config/claw/actions.tsv`, lazy-sourcing `obsidian.zsh` + `clin.zsh`), and install rows in the `cortex` + `research` toolchains (`cargo:clin-rs`).
...
- `shell/claw-login.zsh` — the login DECIDER (`claw_login`, step 2b) plus the one-shot `precmd` render: attention strip, the once-a-day card, the background cache kicks, OSC. No fzf, no stdin, actor-gated.
- `shell/claw-palette.zsh` — `claw_palette`: ONE flat frecency-ranked fzf list of every destination, opened by bare `claw`, `claw menu` or `^G` (`CLAW_PALETTE_KEY`). `enter` runs, `ctrl-p` pins a profile as the login default, `esc` closes and applies nothing. `_claw_apply_outcome` / `_claw_action` are the appliers.
- `scripts/utils/registry.sh` + `config/claw/actions.tsv` — the ONE registry the palette, `claw help`, completion and `claw tui-stats` are generated from.
- `scripts/utils/claw-dashboard.py` — the ONE renderer (`--login` / `--profile <key>`).
...
- **SSH + agent safety:** `claw_login` returns having done nothing in non-interactive shells, with piped stdin, inside an SSH session whose stdout is not a tty, or under `TERM=dumb`; a nested shell (one that inherited `CLAW_ACTIVE_PROFILE`) never renders or relocates. Agent (`CLAUDECODE`) and IDE-terminal shells register no hook at all. An SSH login gets the attention strip and the background kicks but no card and no OSC — and no surface on the login path ever reads stdin, which is why the fzf picker moved to `claw_palette`. load-env.zsh is silent. No stdout pollution.
...
- **FZF integration:** the `claw` palette, plus fuzzy selection in git branches, k8s contexts, AWS profiles, process killing, tunnel manager — never on the login path
...
| `shell/claw-fn.zsh` | The one `claw()` zsh function; `_claw_load_profile`, the one load path |
| `shell/claw-login.zsh` | `claw_login` (decide only, step 2b) + the one-shot precmd render |
| `shell/claw-palette.zsh` | `claw_palette` (bare `claw` / `^G`) + `_claw_apply_outcome` / `_claw_action` |
| `scripts/utils/registry.sh` | The one registry emitter (rows/palette/completion/ids/help/check) |
| `config/claw/actions.tsv` | Verb half of the registry (8 columns; profiles are the other half) |
| `scripts/utils/claw-dashboard.py` | The one renderer — `--login` card, `--profile <key>` card |
