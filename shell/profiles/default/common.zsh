...
default-help() {
    ...
    echo "  ${c_white}claw <profile>${c_reset}  ${c_dim}Switch now (e.g. claw cloud) — same as claw load${c_reset}"
    echo "  ${c_white}claw${c_reset}            ${c_dim}Palette: every profile + verb (also ${c_white}^G${c_dim})${c_reset}"
    echo "  ${c_white}claw pin <p>${c_reset}    ${c_dim}Make it the login default${c_reset}"
    echo "  ${c_white}claw off${c_reset}        ${c_dim}Unload the profile${c_reset}"
    echo ""
    echo "  ${c_purple}${c_bold}AI agents${c_reset} ${c_dim}(registered in agents.toml)${c_reset}"
