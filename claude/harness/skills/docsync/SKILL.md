...
description: >
  ...
  Use whenever an agent needs to create Things 3 todos tied to a repo, onboard a
  repo, or mirror a repo's key markdown into the vault. Replaces ad-hoc add_todo
  calls that silently land in the Inbox. Trigger on "docsync", "sync this repo to
  Things", "create a todo for {repo}", sprint planning that emits tasks, or any
  repo-scoped task capture.
---
...
### 2. `add_todo_for(repo, task_name, task_id=None, **kw)`

```
r = resolve_project(repo)

if r.status in ("bound", "exact", "created"):
    add_todo(title=task_name, list_id=r.uuid, **kw)        # lands in the project
    ...
    add_todo(title=label, **kw)                             # Inbox (no list_id)
    PROMPT_USER(
...
### 3. GUARD — fail loud, never silent-Inbox-on-error
...
### 4. Artifact mirror (one-way repo -> vault) — runs in /sync-docs APPLY mode
