#!/bin/bash

################################################################################
# Research Toolchain Installer
# Description: Installs Research, OSINT, Data Extractor, and Parsing tools
################################################################################

set -e
set -u

RED='\033[0;31m'
GREEN='\033[0;32m'
BLUE='\033[0;34m'
NC='\033[0m'

echo -e "${BLUE}[INFO]${NC} Installing Research Toolchain..."

tools=(
    "curl"
    "httpie"
    "jq"
    "csvkit"
    "ripgrep"
    "awk"
    "gnu-sed"
    "pup"
    "htmlq"
    "yt-dlp"
    "tldr"
    "fzf"
    "wget"
    "aria2"
    "pandoc"
)

for tool in "${tools[@]}"; do
    if brew list "$tool" &>/dev/null; then
        echo -e "${BLUE}[INFO]${NC} $tool already installed."
    else
        echo -e "${BLUE}[INFO]${NC} Installing $tool..."
        brew install "$tool" || echo -e "${RED}[ERROR]${NC} Failed to install $tool"
    fi
done

# Python based research tools
echo -e "${BLUE}[INFO]${NC} Installing Python Research Tools via pipx..."
if ! command -v pipx &> /dev/null; then
    brew install pipx
    pipx ensurepath
fi

pipx_tools=(
    "scrapy"
    "wordfreq"
    "textract"
    "polyglot"
)

for tool in "${pipx_tools[@]}"; do
    echo -e "${BLUE}[INFO]${NC} Ensuring pipx install for $tool..."
    pipx install "$tool" || echo -e "${RED}[ERROR]${NC} Failed to install $tool via pipx"
done

echo -e "${GREEN}[SUCCESS]${NC} Research Toolchain installed."
